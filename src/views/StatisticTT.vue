<template>
  <div class="class-manager">
    <h2 class="title">Thống kê đợt thực tập</h2>
    ...
    const getListSemeters = (page, pageSize) => {
      if (!isRequestAPI.value) {
        if (getData(ACCESS_TOKEN, "")) {
          ...
          axios
            .get(
              `${environment.API_URL}${ENDPOINT.semesters.index}?page=${page}&page-size=${pageSize}&type=INTERNSHIP&status=CLOSE`,
              config
            )
            .then((res) => {
              if (res.data.success) {
        ...
        if (getData(ACCESS_TOKEN, "")) {
          ...
          axios
            .get(
              ...
              }?page=${page}&page-size=${pageSize}&type=INTERNSHIP${
                searchSemester.value.year
